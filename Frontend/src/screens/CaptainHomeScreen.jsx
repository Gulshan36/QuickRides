...
function CaptainHomeScreen() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Emit location to server to update captain position and status
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              ...
              showAlert('Location Permission', 'Please enable location access to receive ride requests', 'failure');
              break;
      ...
      showAlert('Geolocation Not Supported', 'Your browser does not support geolocation', 'failure');
    }
  };
  ...
  // Set up socket listeners once (independent of captain state)
  useEffect(() => {
  ...
  // Handle captain connection and location updates
  useEffect(() => {
    if (captain._id) {
      Console.log("👤 Captain ID available, joining room...");
      ...
      // Update location every 10 seconds to keep captain active and location fresh
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10 bg-green-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
        <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
        <span className="text-sm font-semibold">Active - Receiving Rides</span>
      </div>
      ...
      {showCaptainDetailsPanel && !showNewRidePanel && (
        <div className="absolute bottom-0 flex flex-col justify-start p-4 gap-2 rounded-t-lg bg-white h-fit w-full transition-all duration-300">
          {/* Driver details */}
