...
function UserHomeScreen() {
  ...
  const [currentLocation, setCurrentLocation] = useState(null); // Store current GPS coordinates
        ...
        async (position) => {
          ...
          try {
            ...
            if (response.data && response.data.address) {
              // Set pickup location from current GPS
  ...
  const createRide = async () => {
    try {
      ...
      Console.log(`  Searching for ${selectedVehicle} captains...`);
  ...
  const setDefaults = () => {
    ...
    // Refresh current location after ride ends
    getCurrentLocation();
  };
  ...
  useEffect(() => {
    ...
    socket.on("ride-confirmed", (data) => {
      ...
      Console.log("Captain location:", data.captain.location);
      
      // Update map to show captain's current location
      setMapLocation(
      ...
      localStorage.setItem("confirmedRideData", JSON.stringify(data));
      
      // Update UI to show ride details panel with captain info
      setRideCreated(false);
      setShowFindTripPanel(false);
      setShowSelectVehiclePanel(false);
      setShowRideDetailsPanel(true);
      
      // Update panel state in localStorage
      localStorage.setItem("panelDetails", JSON.stringify({
        showFindTripPanel: false,
        showSelectVehiclePanel: false,
        showRideDetailsPanel: true
      }));
      
      Console.log("✅ UI updated - showing ride details with captain info");
    });

    socket.on("ride-started", (data) => {
      Console.log("🚀 Ride Started - OTP Verified");
      Console.log("Ride data received:", data);
      Console.log("Route:", data.pickup, "to", data.destination);
      
      // Update map to show full route from pickup to destination
      setMapLocation(
        `https://www.google.com/maps?q=${data.pickup} to ${data.destination}&output=embed`
      );
      
      // Update confirmed ride data with started ride info
      setConfirmedRideData(data);
      localStorage.setItem("confirmedRideData", JSON.stringify(data));
      
      // Force all panels to correct state
      Console.log("Setting panels - RideDetails: true, FindTrip: false, SelectVehicle: false");
      setShowFindTripPanel(false);
      setShowSelectVehiclePanel(false);
      setRideCreated(false);
      setShowRideDetailsPanel(true); // Set this last to ensure it takes effect
      ...
      Console.log("✅ Live tracking enabled - RideDetails panel should be visible");
      Console.log("showRideDetailsPanel state:", true);
    });
    ...
    socket.on("ride-ended", (data) => {
      ...
      // Update to current location after ride ends
      getCurrentLocation();
    });
  }, [user]);
        ...
        <div className="absolute b-0 flex flex-col justify-start p-4 pb-2 gap-4 rounded-b-lg bg-white h-fit w-full">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="text-xs bg-zinc-100 px-3 py-2 rounded-lg hover:bg-zinc-200 disabled:opacity-50"
            >
          ...
          <div className="flex items-center relative w-full h-fit">
            ...
            <div className="w-full">
              <input
                id="pickup"
                placeholder={fetchingLocation ? "Fetching current location..." : "Add a pick-up location"}
